...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:guess_two) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(guess_two.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:guess_three) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(guess_three.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:guess_four) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(guess_four.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:guess_five) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(guess_five.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:equal_first) { described_class.new(3, 3, number_guessing, 3) }
      ...
      it 'is game over' do
        # allow(equal_first).to receive(:guess).and_return(3)
        expect(equal_first).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:mid_number) { double('random_number', value: 3) }
      subject(:not_equal) { described_class.new(3, 3, mid_number, 4) }

      it 'is not game over' do
        expect(not_equal).to_not be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      # let(:number_range) { double('random_number', value: 3) }
      subject(:guess) { described_class.new(5, 8, number_range, 7) }
      ...
      it 'updates min to the same value as max' do
        expect(guess.min).to eq(guess.max)
      end

      # it 'does not update max' do
      #   expect(guess.max).to eq(8)
      # end
    end

    context 'when guess is 9, min=3, and max=3' do
      # let(:number_range) { double('random_number', value: 0) }
      subject(:guess_two) { described_class.new(3, 3, number_range, 9) }

      before do
        guess_two.update_range
      end

      it 'updates max to 8' do
        expect(guess_two.max).to be(8)
      end

      it 'does not update min' do
        expect(guess_two.min).to eq(3)
      end
    end

    context 'when the guess is 9, min=5, max=8' do
      # let(:number_range) { double('random_number', value: 3) }
      subject(:guess_three) { described_class.new(8, 10, number_range, 9) }

      before do
        guess_three.update_range
      end

      it 'updates max to be the same value as min' do
        expect(guess_three.max).to eq(guess_three.min)
      end
    end
  end
end
