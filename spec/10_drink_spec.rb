...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_preworkout) { described_class.new('preworkout', 16) }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_preworkout.type).to be('preworkout')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_preworkout.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:coconut_juice) { described_class.new('coconut_juice', 16) }
      # remove the 'x' before running this test
      it 'is full' do
        expect(coconut_juice).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:milk) { described_class.new('milk', 7) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(milk).to_not be_full
      end
    end
  end
end
